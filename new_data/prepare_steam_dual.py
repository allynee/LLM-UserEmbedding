...
Outputs (under --data-dir, default ./data/steam):
...
def parse_steam_date(date_str):
    """
    Parses date string like '2017-12-17' to unix timestamp.
...
def main():
    ...
    ap.add_argument(
        "--raw",
        default="./data/raw/steam_reviews.json",
        help="Path to Steam reviews raw file (steam_reviews.json or .json.gz)",
    )
    ap.add_argument(
        "--data-dir",
        default="./data/steam",
    ...
    ap.add_argument(
        "--user-map",
        default="./data/mapper/steam_user.json",
        help="JSON mapping internal_user_id -> original username",
    )
    ap.add_argument(
        "--item-map",
        default="./data/mapper/steam_item.json",
        help="JSON mapping internal_item_id -> original product_id",
    )
    ap.add_argument(
        "--short-k",
        type=int,
    ...
    orig2user = load_and_invert_mapping(user_map_path, id_field="uid", orig_field="username")
    orig2item = load_and_invert_mapping(item_map_path, id_field="iid", orig_field="product_id")

    # 3) stream raw data
    print(f"Streaming raw Steam file from {raw_path} ...")
    ...
    with smart_open(raw_path) as f:
        for line in f:
            ...
            # Steam data uses Python literal syntax (single quotes, u'string')
            try:
                obj = ast.literal_eval(line)
            except (ValueError, SyntaxError):
                # Fallback to json loads if it happens to be standard json
                try:
                    obj = json.loads(line)
                except json.JSONDecodeError:
                    continue

            u_orig = obj.get("username")
            i_orig = obj.get("product_id")
            
            # Some records might use 'user_id' instead of username if username is missing?
            # But mapper uses username.

...
def _load_modelconf(model_full: str) -> None:
    """
    Load encoder/config/modelconf/{model_full}.yml and merge into configs.
    """
...
def _choose_checkpoint(model_full: str, dataset: str, explicit: Optional[str]) -> Path:
    ...
    # Prefer model_full-dataset-*.pth; fall back to any .pth
    cands = sorted(
        ckpt_dir.glob(f"{model_full}-{dataset}-*.pth"),
...
def _per_user_metrics(
    top_items: np.ndarray, gt_items: List[int], ks=(5, 10, 20)
...
def main():
    ap = argparse.ArgumentParser(
        description="Dump per-user recs & metrics for a saved checkpoint."
    )
    ap.add_argument("--dataset", default="amazon", help="amazon | yelp | steam")
    ap.add_argument(
        "--model",
        required=True,
        help="Full model name, e.g. lightgcn_fusion, lightgcn_plus_fusion, lightgcn_gene_fusion",
    )
    ...
    # Load modelconf YAML
    _load_modelconf(model_full)

    # Override dataset name AFTER loading model config
    configs["data"]["name"] = dataset

    # Ensure test ks exist
    ...
    print(f"\nWrote per-user CSV to {out_csv}")

    # Sanity check against Metric().eval
    agg_recalls = (sum_recalls / n_users_eval)
    agg_ndcgs = (sum_ndcgs / n_users_eval)
    print("[Our aggregate from CSV]")
    print(
        f"  recall@5={agg_recalls[0]:.6f}  "
        f"recall@10={agg_recalls[1]:.6f}  "
        f"recall@20={agg_recalls[2]:.6f}"
    )
    print(
        f"  ndcg@5={agg_ndcgs[0]:.6f}    "
        f"ndcg@10={agg_ndcgs[1]:.6f}    "
        f"ndcg@20={agg_ndcgs[2]:.6f}"
    )

    official = Metric().eval(model, data_handler.test_dataloader)
    print("[Official Metric().eval]")
    print(
        f"  recall@5={official['recall'][0]:.6f}  "
        f"recall@10={official['recall'][1]:.6f}  "
        f"recall@20={official['recall'][2]:.6f}"
    )
    print(
        f"  ndcg@5={official['ndcg'][0]:.6f}    "
        f"ndcg@10={official['ndcg'][1]:.6f}    "
        f"ndcg@20={official['ndcg'][2]:.6f}"
    )
